import Book1 from '../../../assets/books/book1.jpg';
import Book2 from '../../../assets/books/book2.jpg';
import Book3 from '../../../assets/books/book3.jpg';
import Book4 from '../../../assets/books/book4.jpg';
import Book5 from '../../../assets/books/book5.jpg';
const rebooksdata = {
  booklist: [
    {
      bookimg: Book1,
      booktitle: '你不知道的Chrome调试技巧',
      bookintroduction: '熟练掌握Chrome调试技巧，直接提升工作效率',
      bookauthor: 'dendoink',
      bookauthorintroduction:
        "Tomek Sulkowski：Angular Tricity 联合创始人/'Angular'…",
      bookprice: '￥0.01',
      bookbuyer: '18小节·0人已购买'
    },
    {
      bookimg: Book2,
      booktitle: '前端面试之道',
      bookintroduction:
        '祝你建立起完整的前端知识架构体系，探究知识的原理，深入了解大肠常考知识点',
      bookauthor: 'yck',
      bookauthorintroduction: '宋小菜高级开发，GitHub 15K Start项目作者…',
      bookprice: '￥49.9',
      bookbuyer: '36小节·阅读时长265分·0人已购买'
    },
    {
      bookimg: Book3,
      booktitle: 'React实战，设计模式和最佳实践',
      bookintroduction:
        '深入链接React应用中的设计模式，总结业界验证的最佳实践…',
      bookauthor: '程墨',
      bookauthorintroduction: 'Hulu架构师',
      bookprice: '￥29.9',
      bookbuyer: '21小节·阅读时长153分·0人已购买'
    },
    {
      bookimg: Book4,
      booktitle: 'Taro多端开发实现原理与项目实战',
      bookintroduction:
        '浦西Taro多端开发框架的实现原理，并通过电商核心的项目实战…',
      bookauthor: '凹凸实验室',
      bookauthorintroduction: '凹凸实验室',
      bookprice: '￥9.9',
      bookbuyer: '27小节·阅读时长205分·0人已购买'
    },
    {
      bookimg: Book5,
      booktitle: '前端性能优化原理与实践',
      bookintroduction:
        '毫秒必争！深入理解前端性能原理，将晦涩的知识转换为可爱的生产力',
      bookauthor: '修言',
      bookauthorintroduction: '阿里巴巴集团前端工程师',
      bookprice: '￥19.9',
      bookbuyer: '15小节·阅读时长130分·0人已购买'
    }
  ]
};
export default rebooksdata;
